...
- **`email`**: fabrique le courriel de diffusion, un `.msg` Outlook prêt à compléter et à envoyer, à partir du texte validé et des deux visuels collés dans la conversation. Corps HTML au gabarit, images affichées dans le corps, typographie française appliquée. La même passe écrit trois artefacts conservables: `pastille.html`, aplati et coloré, qui s'importe dans Notion; `courriel.html`, trace fidèle de l'envoi; `pastille.md` (voir « Les artefacts produits »).
...
| `pastille.html` | **importer dans Notion**, lire, conserver | HTML sémantique **sans aucune table**, habillé aux teintes de la série; visuels incorporés, donc un seul fichier à importer, sans archive |
...
**Pour Notion: `pastille.html`, tel quel, sans zip.** Validé à l'usage. Ce que Notion aplatit mal, ce sont les tables de mise en page, et cet artefact n'en a aucune: `h1` pour le titre, `p` pour les paragraphes, `blockquote` plus `ul` pour l'encadré, `figure` et `figcaption` pour le schéma et sa légende. Ses visuels étant incorporés, il n'y a ni archive à préparer ni image à retrouver. **N'importez pas `courriel.html`**, dont les sept tables imbriquées sont exactement le cas que la documentation de Notion signale.
...
Le même fichier sert donc à l'import et à la lecture: `pastille.html` reprend les teintes du courriel (bandeau bleu au numéro orange, encadré bleu pâle à puces bleues, bloc annexe à barre latérale, texte justifié) en styles **en ligne**. Ce choix est délibéré: une feuille `<style>` serait ignorée par un importeur, et un importeur naïf pourrait en recracher le contenu dans la page. Notion, lui, ne garde probablement que la structure; les couleurs valent pour la lecture du fichier dans un navigateur. Sa palette de texte est d'ailleurs une liste de noms, pas des valeurs hexadécimales, donc les teintes exactes de la charte n'y sont pas reproductibles, même par l'API.
