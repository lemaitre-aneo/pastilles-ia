...
- **Le sujet du courriel est le seul texte accentué du fichier**, le corps partant en entités ASCII. Outlook (new) reconvertit le `.msg` en MIME, rabat ce sujet en octets cp1252, puis le relit dans un codage sur deux octets avec repli sur cp1252 quand le décodage échoue: un sujet qui s'y décode entièrement s'affiche `[Pastille IA de l掗t閉 #7 : ...`, un sujet qui le fait échouer est intégralement sauvé. Il suffit d'un accent suivi d'une espace ou d'une ponctuation pour cela, et c'est le rôle de l'année dans le préfixe de série: sur les 45 titres, 29 sont corrompus avec `[Pastille IA de l'été]`, aucun avec `[Pastille IA de l'été 2026]`. `build.py` alerte en affichant le rendu erroné, `verify.py` refuse le fichier.
