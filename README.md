...
Ce que Notion garde: la structure des blocs. Ce qu'il ne garde pas: les couleurs. Seule la légende y ressort en gris, et c'est Notion qui colore nativement ses légendes, pas un style qui survivrait. Défaut connu et assumé de cet import: la légende du schéma y arrive **en double**, en paragraphe avant l'image et en légende native sous elle, l'importeur lisant deux fois une `figcaption`. Supprimer le paragraphe en trop suffit; le skill `email` dit pourquoi, et ce qui a été essayé pour l'éviter. L'habillage aux teintes de la série vaut donc pour la lecture du fichier dans un navigateur; sa palette de texte étant une liste de noms et non des valeurs hexadécimales, les teintes exactes de la charte n'y seraient de toute façon pas reproductibles.
