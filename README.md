...
- **`generate`**: crée une pastille à partir d'un titre. Lance de vrais sous-agents en parallèle (cinq rédacteurs sous des angles différents, fusion, puis revue critique par trois relecteurs et correction). C'est aussi lui qui régénère une pastille existante quand la demande réclame du matériau neuf, un changement d'axe par exemple; le nouvel axe est alors partagé par les cinq brouillons, qui gardent leur jeu d'angles. Pour un seul morceau à reprendre, il sait aussi lancer un fan-out ciblé, trois rédacteurs sur ce fragment.
...
Les cinq rédacteurs ne reçoivent pas un jeu d'angles figé. **Deux slots de noyau** sont toujours là, mécanique et enjeu: sans le premier aucun brouillon ne porte la justesse technique, et le second fournit le dernier paragraphe, obligatoire dans la série. **Trois slots libres** se choisissent dans la bibliothèque d'angles (analogie, cas d'usage, idée reçue, contre-exemple, avant/après, ordre de grandeur, frontière, filiation, scène, garde-fou) selon l'axe et la rubrique, ou s'inventent quand un axe le mérite.

Le jeu par défaut (analogie, cas d'usage, idée reçue, mécanique, enjeu) reste le repli: il est éprouvé sur la série, s'en écarter demande une raison. Et la liberté est bornée par deux garde-fous, sans lesquels elle se retourne contre le fan-out:

- **Contrainte de diversité**: trois slots libres qui ouvrent sur la même porte ne valent qu'un brouillon payé trois fois.
- **Ne pas choisir les angles d'après ce qu'on écrirait soi-même**, sinon cinq points de vue deviennent cinq versions du même. L'angle qui parait le moins naturel est souvent le plus rentable.
...
| **Reprise ciblée**: un morceau délimité est à re-produire (paragraphe qui n'explique rien, analogie qui tombe à plat, encadré à refaire) mais le reste tient | Petit fan-out sur ce seul morceau, avec le texte conservé transmis comme cadre. Trois rédacteurs, pas cinq: rien de validé n'est jeté. |
| **Changement d'angle**: le traitement change (partir d'un cas d'usage, d'une idée reçue, du mécanisme) mais l'axe reste | Reprise du brouillon écrit sous cet angle, s'il faisait partie du jeu retenu. Zéro sous-agent. Fan-out sous angle imposé seulement si cet angle n'a pas été couvert. |
