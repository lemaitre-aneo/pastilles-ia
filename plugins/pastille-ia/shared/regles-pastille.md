...
- **À essayer**, l'encadré actionnable (style `essayer`, teinte orange): l'explication de ce que le lecteur peut tenter dès son prochain échange avec un modèle, dite avec ses mots à lui. Il se mesure à ceci: il peut le faire aujourd'hui, sans rien installer ni demander à personne. **Ce n'est pas un prompt à recopier**, et c'est un parti pris: un prompt collé se périme avec les modèles, se copie mal depuis un courriel, et ne s'ajuste pas à la tâche de celui qui le lit. Le lecteur qui a compris ce qu'il essaie le refait sur son propre cas; celui qui a collé une formule ne l'a fait qu'une fois. Une tournure du genre « demandez au modèle de citer ses sources avant de conclure » vaut mieux qu'un bloc de prompt entre guillemets.
...
**Forme**: une à deux phrases, cinquante mots au maximum, en prose rédigée, sans exception. Le bloc annexe suit le corps de la pastille sur ce point: pas de liste, pas de bloc de code, pas de formule à recopier telle quelle.
...
L'ordre de génération encadre le gabarit, il ne le termine pas. Le bloc décrit deux images et n'en demande qu'une: cette contrainte s'énonce donc avant les deux descriptifs et se rappelle après, faute de quoi elle doit annuler la spec du schéma juste après l'avoir posée, et c'est le descriptif le plus dense et le plus frais qui l'emporte. Trois précautions vont avec. La consigne est positive et nomme l'image ("l'illustration-titre") plutôt que son rang, un ordinal se réévaluant à la lecture. Le descriptif du schéma est marqué comme contexte, sur le modèle du texte de la pastille "à NE PAS afficher", qui lui est bien respecté. Et le sens de la dépendance est dit: l'illustration-titre fixe l'identité visuelle, le schéma la reprend ensuite, ce que la génération séparée permet puisque la seconde image peut prendre la première en référence. Sans cela, la charte commune se lit comme un système à établir d'abord, et commencer par le schéma devient défendable.
...
```
Deux images sont décrites ci-dessous, mais tu n'en génères qu'une maintenant: l'illustration-titre. C'est elle qui fixe l'identité visuelle, le schéma la reprendra ensuite, quand je te le demanderai. Les deux images vont dans deux fichiers distincts et partagent cette charte graphique: [bloc Charte graphique]. Tout texte affiché dans les images est en français.
Contexte pour comprendre le sujet, à NE PAS afficher dans les images: [ici le texte complet de la pastille].
Image 1, illustration-titre, la seule à générer maintenant: composition épurée et moderne, focus graphique central iconique qui illustre le sujet. Ce n'est pas un schéma de processus. Seul texte à afficher: le titre, en en-tête, sans faute d'orthographe, police sans serif corporate, sans sous-titre ni texte secondaire. Le titre exact: "TITRE EXACT". Format 16:9.
Image 2, schéma explicatif, décrite ici pour contexte et à NE PAS générer maintenant, dans son propre fichier, cohérente avec l'illustration-titre une fois celle-ci produite: diagramme d'entreprise propre et net, [FORME retenue, décrite pour être dessinée sans ambiguïté: par exemple "une pile de trois couches, la plus large en bas", "une matrice à deux axes et quatre cases", "une boucle en quatre temps", "deux colonnes comparées ligne à ligne"], qui donne à voir [le mécanisme, en une phrase]. Éléments et disposition: [décrire les blocs ou zones dans l'ordre où on les lit]. Libellés exacts à afficher en français: [liste des libellés]. Très peu de fioritures, focus sur la clarté, n'inclus pas le titre. Format [FORMAT retenu: 4:3, carré ou portrait 4:5, selon ce que la forme réclame], libellés assez grands pour rester lisibles une fois l'image réduite à 560 pixels de large.
Rappel: dans cette réponse, l'illustration-titre seulement.
```
...
- Images plafonnées en largeur, 600 pixels pour l'illustration-titre et 560 pour le schéma, hauteur automatique: elles ne sont jamais étirées au-delà de leur taille nominale et se réduisent seulement si la fenêtre passe en dessous. Chaque image porte un texte alternatif renseigné.
- Le schéma porte en plus un plafond de hauteur, 600 pixels, parce que sa forme est libre: en portrait 4:5, 560 pixels de large en font près de 700 de haut, et le visuel remplit l'écran d'un téléphone avant que le texte ne reprenne. Ce plafond ne se pose pas en CSS, pour deux raisons qui se cumulent: `max-height` sur une image dont la largeur est fixée à 100% écrase ses proportions au lieu de la réduire, et Word l'ignore de toute façon, comme il ignore `max-width`. Le rendu calcule donc la largeur d'affichage depuis les proportions réelles du visuel, de sorte que sa hauteur reste sous le plafond; un schéma en paysage n'est jamais concerné et garde ses 560 pixels. Le format du schéma touche donc la largeur à laquelle il s'affiche, jamais ses proportions, et il reste borné par la lisibilité et non par la mise en page (voir « Le schéma: forme libre, invariants fermes »).
