...
**Le sujet a un piège d'encodage, et c'est le préfixe qui décide du sort de toute la série.** Le sujet est le seul texte accentué du courriel, le corps partant en entités ASCII. Or Outlook (new) reconvertit le `.msg` en MIME, rabat ce sujet Unicode en octets cp1252, puis tente de les relire dans un codage sur deux octets, avec repli sur cp1252 quand ce décodage échoue. Le décodage avale les caractères deux par deux: `[Pastille IA de l'été] #7 : Les tokens : la monnaie d'échange` s'affiche `[Pastille IA de l掗t閉 #7 : Les tokens : la monnaie d掗change`.

Il n'échoue, et ne rend donc la main au repli correct, que si la chaîne s'y prête mal: **un seul accent suivi d'un espace ou d'une ponctuation** — un octet inférieur à 0x40 — suffit, et il sauve alors le sujet entier, titre compris. C'est pour cela que le préfixe est décisif: il est sur tous les courriels. S'il porte lui-même cette rupture, aucun titre n'est exposé; s'il n'en porte pas, chaque titre joue sa chance seul, et mesuré sur les 45 titres de la série, 29 sortent corrompus. Un accent en fin de mot suivi d'une espace fait l'affaire, c'est ce qui a protégé « de zéro à chaque session » pendant toute une saison sans que personne le remarque.

Deux précautions sur le remède. Une espace de largeur nulle ne relève pas du même mécanisme: absente de cp1252, elle devient un `?` bien visible si le client substitue, et ne sert que si ce client bascule sur un codage déclaré — cela se vérifie avant d'y compter, et `hors_cp1252` la signale. Et on ne déshabille pas un titre de ses accents: un titre français désaccentué se remarque tout de suite, la faute n'est pas dans la série mais dans le client. `build.py` alerte à la fabrication en affichant le rendu erroné, `verify.py` refuse le fichier.
