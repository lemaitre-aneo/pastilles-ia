...
1. Le contenu de la pastille: titre retenu, les puces de « L'essentiel », les 3 ou 4 paragraphes, la légende du schéma, le texte alternatif du schéma, et le bloc annexe, qui est systématique. S'il vient d'être produit dans la conversation, reprends-le tel quel sans le réécrire. Sinon demande-le, ou demande à l'utilisateur de le recoller. Si la pastille recollée n'a pas de bloc annexe, c'est le cas courant d'un texte antérieur à cette règle: `build.py` ne bloque pas, il le signale et propose de quoi l'écrire, donc c'est à toi de ne pas laisser passer l'avertissement. Propose un bloc tiré du texte, dans l'une des deux catégories de la série ("À essayer" ou "Le piège", voir la spec partagée, section « Le bloc annexe »), et fais-le valider avant de construire. Tu ne réécris pas la pastille pour autant: un bloc de deux phrases se soumet, il ne s'impose pas. Si l'utilisateur préfère s'en passer, construis sans, en disant que le fichier part sans son bloc de clôture. Si la légende du schéma manque, demande-la: ne l'invente pas, elle dit ce qu'il faut retenir d'un visuel que tu ne peux pas regarder. Le texte alternatif se traite autrement quand l'aperçu des visuels est là: dérive-le de l'aperçu (ce qui est dessiné, la forme et les libellés dans l'ordre de lecture, une à deux phrases) et soumets-le, ce n'est pas de l'invention mais une extraction. Sans aperçu, demande-le comme la légende. Et ne recopie jamais l'une dans l'autre: la légende s'imprime sous l'image et dit quoi en retenir, l'alt est ce que reçoit un lecteur d'écran et dit ce qui est dessiné; `build.py` signale les deux textes identiques.
...
- **La légende du schéma arrive en double dans Notion, et c'est assumé.** L'importeur lit une `figcaption` deux fois: une fois en paragraphe placé **avant** l'image, parce qu'il aplatit la figure en collectant son contenu texte, une fois en légende native sous elle. Le nettoyage tient en un geste, supprimer le paragraphe en trop, et c'est le moins coûteux des deux: écrire la légende en `<p>` frère de la figure supprime bien le doublon, mais il faut alors la couper et la coller dans le champ de légende de l'image pour retrouver la légende native. Inutile de chercher mieux, tout a été essayé à l'import: le défaut tient à la `figcaption` elle-même et non au balisage autour (sans blancs entre les balises, figure sans style, forme exacte de l'export Notion: tous dupliquent); il ne se contourne pas en la cachant, l'importeur ignorant le CSS dans les deux sens (un `display:none` n'empêche pas l'import, et une légende posée en CSS `::after` depuis un attribut n'est pas importée du tout); et aucune métadonnée ne l'alimente (`alt` reste un alt et ressort au survol, `title` et `aria-label` sont ignorés). La seule voie vers une légende native sans doublon est l'API Notion, qui ne consomme pas un fichier mais construit la page bloc à bloc: un autre chantier, et un troisième rendu à maintenir.
...
- **Images plafonnées en largeur**, 600 pixels pour l'illustration-titre et 560 pour le schéma, avec hauteur automatique: elles ne sont jamais étirées au-delà de leur taille nominale et se réduisent seulement si la fenêtre passe en dessous. **La légende du schéma partage ce plafond et ce centrage**, donc son bord gauche est celui de l'image: une légende qui dépasse le visuel qu'elle décrit se lit comme un paragraphe de plus. Word ignorant `max-width`, le plafond de la légende lui est donné en plus par commentaire conditionnel, comme celui de la colonne.
- **Le schéma est en plus plafonné en hauteur**, à 600 pixels, sa forme étant libre: un portrait 4:5 affiché sur 560 pixels en fait près de 700 de haut. Le plafond est posé à la source et non en CSS, `max-height` écrasant les proportions d'une image dont la largeur est fixée à 100% et Word l'ignorant comme il ignore `max-width`: `render.largeur_schema` réduit la largeur d'affichage d'après les proportions réelles du visuel, que `build.py` relève après rognage (champ dérivé `_dimensions_schema`, préfixé pour rester hors du dossier incorporé). Un schéma en paysage garde donc ses 560 pixels, et la légende suit la largeur retenue. Sans Pillow, les proportions sont inconnues et le rendu retombe sur la largeur nominale.
