...
- **La légende du schéma est un paragraphe, pas une `figcaption`.** C'est la seule balise de l'artefact qui n'est pas celle du bloc qu'elle décrit, et c'est un contournement: Notion importe une `figcaption` **deux fois**, en paragraphe placé avant l'image et en légende native sous elle. Vérifié à l'import, le défaut tient à la `figcaption` elle-même et non au balisage autour, et il ne se contourne pas en la cachant, l'importeur ignorant le CSS dans les deux sens: un `display:none` n'empêche pas l'import, et une légende posée en CSS `::after` depuis un attribut n'est pas importée du tout. Le schéma reste donc une `<figure>`, mais elle ne contient que l'image, et la légende la suit en `<p>` qui reprend son plafond de largeur. Le rendu navigateur est inchangé.

### Ce que Notion garde, et ce qu'il ne garde pas
La structure des blocs, oui, et c'est le but. Les couleurs, non, aucune: la légende du schéma arrive en paragraphe ordinaire, puisqu'elle n'est plus une `figcaption` (voir plus haut), donc elle n'hérite pas non plus du gris dont Notion colore nativement ses légendes. L'habillage aux teintes de la série vaut donc pour la lecture du fichier dans un navigateur. À noter que la couleur de texte de Notion est une palette de noms, pas des valeurs hexadécimales: les teintes exactes de la charte n'y sont de toute façon pas reproductibles, même en construisant la page par l'API.
