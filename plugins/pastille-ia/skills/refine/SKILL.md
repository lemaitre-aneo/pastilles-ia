...
- Note la rubrique de diffusion. L'artefact la porte (champ `rubrique` du dossier): reprends-la telle quelle, c'est un arbitrage déjà rendu. À défaut, ne te contente pas de la position dans la liste des 45: elle donne le classement d'inventaire, alors que la rubrique suit l'axe et le contenu du texte que tu as sous les yeux (voir la spec partagée, section « Numéro et rubrique »). Lis-la donc sur le texte, et signale l'écart en une ligne s'il y en a un, le temps de lecture et le numéro n'étant pas les seules valeurs du bandeau à pouvoir être fausses.
...
- L'encadré "L'essentiel", puis le texte raffiné, puis le bloc annexe, avec sa catégorie en libellé ("À essayer" ou "Le piège"). Si la pastille fournie n'en comportait pas, rédige-les: ils sont désormais requis par la spec, une reprise étant justement le moment où une pastille ancienne rejoint le formalisme de la série. Dis en une ligne que le bloc annexe est neuf et pourquoi tu as retenu cette catégorie plutôt que l'autre: c'est un ajout, pas une correction, et l'utilisateur doit pouvoir le rediscuter (voir la spec partagée, section « Le bloc annexe »).
- La rubrique de diffusion, sur une ligne, en disant d'où elle vient: reprise du dossier, ou lue sur le texte quand le dossier n'en portait pas. L'utilisateur n'a rien d'autre sous les yeux, et c'est la valeur du bandeau qu'une reprise fausse le plus facilement.
